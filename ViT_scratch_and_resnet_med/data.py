...
def create_splits(dataset_type="3class", percentage:float = 1):
...
#   train_ids = train_ids[:int((len(train_ids)+1) * percentage)]

#   train_ids = train_ids[int((len(train_ids)+1) * 0.05):]
#   val_ids = train_ids[:int((len(train_ids)+1) * 0.05)]
#   print(f"Prev train example count: {prev_train_count}, cur train example count: {len(train_ids)}")
#   # }
