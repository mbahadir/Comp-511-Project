...
from model import ResNetScratch, ResNetDino, ResNet18, VIT
...
def main(args):
    ...
    seed_everything(40) # original: 42
    # }
...
#     device = torch.device(f"cuda:{cuda}" if torch.cuda.is_available() and gpu else "cpu")
    ...
    model = ResNet18(out_dim=int(dataset_type[0]))
#     model = VIT(out_dim=int(dataset_type[0]))
...
#         print(param.requires_grad)
    ...
    best_model = ResNet18(out_dim=int(dataset_type[0]))
#     best_model = VIT(out_dim=int(dataset_type[0]))
