...
from model import ResNetScratch, ResNetDino
...
# plot the train and val loss on the same plot
def plot_losses(train_losses, val_losses, dataset_type):
    plt.ylabel("Loss")
    plt.xlabel("Epoch")
    plt.title(f'Loss - Epoch')
    plt.ylim(top=1.01*max(train_losses+val_losses))
    plt.plot(train_losses, label="Train")
    plt.plot(val_losses, label="Val")
    plt.legend()
    plt.savefig(f"resnet_scratch_loss_plot_{dataset_type}.png")
...
def main(args):
    ...
    model = ResNetScratch(out_dim=int(dataset_type[0]))
    ...
    train_losses, train_accs, val_losses, val_accs = train(device, model, criterion, optimizer, scheduler, train_loader, val_loader, max_epoch, patience, savedir)

    plot_losses(train_losses, val_losses, dataset_type)
